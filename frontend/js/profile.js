(function () {
  ...
  async function showProfile(container) {
    ...
    container.innerHTML = `<div class="page-head"><div><h2>Kişisel bilgiler</h2><p>İletişim bilgilerinizi ve hesap şifrenizi güvenle güncelleyin.</p></div></div>
      <div class="profile-grid">
        ...
        <div class="settings-stack">
          <section class="panel settings-panel"><header class="panel-header"><div><h3>Profil bilgileri</h3><p>Ad, soyad ve iletişim bilgilerinizi değiştirebilirsiniz.</p></div></header><div class="panel-body"><form class="settings-form" id="profile-form"><div class="field"><label for="firstName">Ad</label><input id="firstName" name="firstName" value="${e(user.firstName)}" maxlength="60" required></div><div class="field"><label for="lastName">Soyad</label><input id="lastName" name="lastName" value="${e(user.lastName)}" maxlength="60" required></div><div class="field"><label for="phone">Telefon</label><input id="phone" name="phone" value="${e(user.phone || '')}"></div><div class="field"><label for="address">Adres</label><input id="address" name="address" value="${e(user.address || '')}"></div><p class="form-error field-wide" id="profile-error" hidden></p><div class="form-actions"><button class="button button-primary" type="submit">Bilgileri Kaydet</button></div></form></div></section>
    ...
    profileForm.addEventListener('submit', async (event) => {
      ...
      try { const updated = await Api.patch('/api/users/profile', Object.fromEntries(new FormData(profileForm).entries())); App.updateMe(updated); UI.toast('Profil bilgileriniz güncellendi.'); await App.navigate('profile'); }
